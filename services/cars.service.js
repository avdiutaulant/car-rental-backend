...
const {InvalidYearError, InvalidNumberOfSeatsError,MissingFieldsError } = require('../errors/validation.error');
...
function getCars(reqQuery) {
    ...
    if (reqQuery.numberOfSeats) {
        ...
        if (isNaN(numberOfSeats) || numberOfSeats <= 1) {
            throw new InvalidNumberOfSeatsError("Number of seats must be a valid number greater than 1");
        }
...
async function createCar(reqBody) {
    const { name, pricePerDay, year, color, steeringType, numberOfSeats } = reqBody;

    if (!name || !pricePerDay || !year || !color || !steeringType || !numberOfSeats) {
    ...
    const newCar = { name, pricePerDay, year: yearInt, color, steeringType, numberOfSeats: numberOfSeatsInt };
